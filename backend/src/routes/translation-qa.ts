...
translationQA.post("/", async (c) => {
  try {
    ...
    if (!question || typeof question !== "string") return c.json({ error: "Question is required" }, 400);
    ...
    if (!translation || typeof translation !== "object") return c.json({ error: "Translation data is required" }, 400);
    ...
    if (!textContent || textContent.type !== "text") return c.json({ error: "No response from Claude" }, 500);
    ...
    console.error("Translation Q&A error:", error);
    if (error instanceof Anthropic.APIError) {
      return c.json({ error: "Claude API error: " + error.message }, (error.status as 400 | 500) || 500);
    }
